...
Full programmatic access to every Stripe API endpoint the pinned stripe-node SDK exposes, across many accounts (standalone keys and Connect), replacing the single-account MCP connector. The engine binary owns operation-level enforcement (classification, confirmation, arming, fan-out refusal, bulk scope review). It cannot defend against a caller that writes its own flags, so a PreToolUse hook in `hooks/` owns the permission-mode boundary; the engine additionally requires a live operation to carry a vetting token (`src/vetting.js`, exit code 15) that only that hook (or a human running `stripe-x vet` from a real terminal) can issue. Both share `src/classify.js`, so a new money-mover added to the frozen DESTRUCTIVE list teaches both gates at once.

KNOWN GAP, do not describe this layer as stronger than it is. The vetting token is scoped to the session and a five minute window, not to the individual call. It proves that some recognised stripe-x invocation happened recently, which ordinary reads keep true almost continuously, so it does NOT prove that this particular call was seen by a gate. Arming also returns before the vetting check, and `runCliBridge` reaches neither. Closing this means binding the token to a canonical key (account, resource, action, live flag) that the hook and the engine compute independently. See docs/SESSION-HANDOFF-2026-08-02.md.
