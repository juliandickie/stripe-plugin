...
- Live mutating and destructive operations now also require a vetting token (`src/vetting.js`), proof that some permission gate saw the call, on top of arming. The `PreToolUse` guard issues one automatically whenever it recognises a stripe-x invocation and does not deny it, so a live call is refused unless some permission gate saw a stripe-x invocation recently. Outside Claude Code, the new `stripe-x vet` subcommand issues a token from an interactive terminal; it requires a real TTY. A live call carrying no token is refused with exit code 15. Known limitation, stated plainly: the token is scoped to the session and a five minute window rather than to the individual call, so it does not yet prove that this specific operation was reviewed, and arming and the `stripe-x cli` bridge do not consult it at all.
