...
- Live mutating and destructive operations now also require a vetting token (`src/vetting.js`), proof that some permission gate saw the call, on top of arming. The `PreToolUse` guard issues one automatically whenever it recognises a stripe-x invocation and does not deny it, so a shell construct the guard cannot read (and therefore cannot classify as live) is refused for lack of a token instead of running unrecognised. Outside Claude Code, the new `stripe-x vet` subcommand issues a token from an interactive terminal; it requires a real TTY, so it cannot be minted by an unattended agent. A live call carrying no token is refused with exit code 15.
