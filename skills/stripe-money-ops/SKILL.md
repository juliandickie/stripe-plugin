...
3. Only after explicit user approval in the same turn, re-run with `--confirm`. For live, this is three separate calls inside Claude Code, each its own approval: arm with `--live --arm-live` first (it arms and exits, exit 14), preview with `--live` alone (step 2 above), then execute with `--live --confirm`. The PreToolUse guard issues the vetting token each of these needs automatically, so no separate vetting step is needed here. If a live call is ever refused with exit 15, that means it carried no vetting token because no permission gate recognised it as this engine (most likely an unusual shell construct); the fix is to run the plain operation again, not to retry the same unrecognised form. Working directly in a terminal with no guard active, re-running the same command changes nothing; run `stripe-x vet` first, which needs a real terminal.
