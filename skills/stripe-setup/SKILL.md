...
Live operations also require a vetting token bound to that exact operation, on top of arming. This covers live writes, live arming, and live calls through the `stripe-x cli` bridge. Inside Claude Code the PreToolUse guard hook (step 7 above) issues one automatically. A human working directly in a terminal, outside Claude Code, must run `stripe-x vet <resource> <action> --live` first, naming the same account, target id and registry as the call it authorises (add `--arm-live` to vet the arming step, which is a separate authorisation). It requires a real terminal (a TTY) and refuses otherwise, which is what stops an unattended agent from minting one for itself. Treat that TTY check as a presence heuristic rather than proof of a human.
